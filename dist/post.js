...
var editBtn = document.getElementById("post-edit-btn");
var editForm = document.getElementById("post-edit-form");
...
var editCancelButton = document.getElementById("edit-cancel");
var authorId = null;
/** @type {FurzonaPost?} */
...
function renderPost(post) {
  ...
  if (editBtn) {
    var me = furzona.user;
    var isOwner = !!user.o || !!(me && user.id === me.id);
...
function applyPostEdits(post) {
  ...
  titleEl.textContent = post.t || "Untitled";
  textEl.textContent = [post.c, post.d].filter(Boolean).join("\n\n");
  var date = new Date(post.updatedAt || post.createdAt);
  metaEl.textContent = "ID: ".concat(post.id, " \u2022 ").concat(date.toLocaleString());
}
var showEditStatus = function showEditStatus(message) {
    ...
    var _ref2 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2(event) {
      ...
      return _regenerator().w(function (_context2) {
        while (1) switch (_context2.p = _context2.n) {
          ...
          case 1:
            button = editForm.querySelector('button[type="submit"]');
            button.disabled = true;
            ...
            _context2.n = 3;
            return furzona.updatePost(id, fields);
          case 3:
            updated = _context2.v;
            if (updated && _typeof(updated) === "object" && updated.id) {
              applyPostEdits(_objectSpread(_objectSpread({}, currentPost), updated));
            } else {
              applyPostEdits(_objectSpread(_objectSpread({}, currentPost), {}, {
                t: fields.title,
                c: fields.text
              }));
            }
          ...
          case 4:
            _context2.p = 4;
            _t2 = _context2.v;
            console.error("Failed to update post:", _t2);
            showEditStatus(_t2.message || "Could not update post.");
          case 5:
            _context2.p = 5;
            button.disabled = false;
